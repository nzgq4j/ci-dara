...
export const FINDINGS: Finding[] = [
  ...
  },
  {
    ...
    remediation: 'Completed (this session): (a) shared newImplicitAuthClient() (anon key, flowType:"implicit", no session persistence) backs resetPasswordForEmail + signUp; (b) app/auth/confirm/route.ts split into GET (renders a branded interstitial, no verify) + POST (verifyOtp → finalizeSignIn, 303). No new env/infra; still uses Supabase’s built-in (Resend-SMTP) email. Links minted before the fix remain dead — request a fresh one. Magic-link (signInWithEmail) + email-change (updateEmail) still run on the PKCE SSR client; they route through the same hardened /auth/confirm but should also move to newImplicitAuthClient() if enabled.',
