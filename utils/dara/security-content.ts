...
export const FINDINGS: Finding[] = [
  ...
  },
  {
    ...
    evidence: 'Two compounding causes. (1) resetPasswordForEmail / signUp ran on the PKCE SSR client, so the email’s {{ .TokenHash }} was a PKCE code (token_hash=pkce_…) that /auth/confirm’s verifyOtp cannot verify without the origin browser’s code-verifier cookie. (2) Even after that was fixed (plain token), prod logs showed the single-use OTP being consumed by an automated email-scanner prefetch (Outlook Safe Links / Defender HEAD+GET the link before the user clicks) — because /auth/confirm ran verifyOtp on the initial GET/HEAD, the scanner burned the token and the user’s real click got "Email link is invalid or has expired".',
    impact: 'Resolved end-to-end. (1) A shared implicit-flow supabase-js client makes {{ .TokenHash }} a plain OTP hash. (2) /auth/confirm no longer verifies on GET/HEAD — it renders an interstitial and only calls verifyOtp on the POST a human triggers by clicking Continue, so link scanners can’t consume the token. Recovery + sign-up confirmation now complete cross-device.',
    remediation: 'Completed (this session): (a) shared newImplicitAuthClient() (anon key, flowType:"implicit", no session persistence) backs resetPasswordForEmail + signUp; (b) app/auth/confirm/route.ts split into GET (renders a branded interstitial, no verify) + POST (verifyOtp → 303); (c) a RECOVERY verify now FORCES the reset before app access — it sets a short-lived httpOnly marker cookie and lands the user on /signin/update_password; the middleware routes every /app request back to that screen until updatePassword() clears the marker, so the reset can’t be skipped by navigating into the app. No new env/infra; still uses Supabase’s built-in (Resend-SMTP) email. Links minted before the fix remain dead — request a fresh one. Magic-link (signInWithEmail) + email-change (updateEmail) still run on the PKCE SSR client; they route through the same hardened /auth/confirm but should also move to newImplicitAuthClient() if enabled.',
